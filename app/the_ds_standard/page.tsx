          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {standardSizes.map((size, index) => (
              <AnimatedSection key={size.name} delay={index * 100}>
                <div
                  className={`relative p-8 rounded-3xl border transition-all duration-500 hover:shadow-xl ${size.highlight
                      ? "bg-primary text-primary-foreground border-primary"
                      : "bg-card border-border hover:border-primary/30"
                    }`}
                >
                  <div
                    className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-medium ${size.highlight
